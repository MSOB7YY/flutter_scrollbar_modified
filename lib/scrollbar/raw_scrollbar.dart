...
class ScrollbarPainter extends ChangeNotifier implements CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    if (_lastAxisDirection == null || _lastMetrics == null || _lastMetrics!.maxScrollExtent <= _lastMetrics!.minScrollExtent) {
      return;
    }
  ...
  double getThumbScrollOffset() {
    ...
    final double fractionPast = (scrollableExtent > 0) ? clampDouble(_lastMetrics!.pixels / scrollableExtent, 0.0, 1.0) : 0;
  ...
  double _getScrollToTrack(ScrollMetrics metrics, double thumbExtent) {
    ...
    final double fractionPast = (scrollableExtent > 0) ? clampDouble((metrics.pixels - metrics.minScrollExtent) / scrollableExtent, 0.0, 1.0) : 0;
...
/// [CustomScrollView], wrap the scroll view widget in a [RawScrollbarModified] widget.
///
...
///
/// If the child [ScrollView] is infinitely long, the [RawScrollbarModified] will not be
...
///
/// {@tool dartpad}
/// This sample shows a [RawScrollbarModified] that executes a fade animation as
/// scrolling occurs. The RawScrollbarModified will fade into view as the user scrolls,
...
/// `controller` for both the [RawScrollbarModified] and the [GridView].
...
class RawScrollbarModified extends StatefulWidget {
  ...
  /// See also:
  ///
  ///   * [RawScrollbarModifiedState.showScrollbar], an overridable getter which uses
  ...
  ///     a subtree.
  ///
  /// Replaces deprecated [thumbVisibility].
  /// {@endtemplate}
  ///
  ...
  /// thumb in a [RawScrollbarModified].
  ///
  ...
  /// See also:
  ///
  ///   * [RawScrollbarModifiedState.enableGestures], an overridable getter which uses
  ///     this value to override the default behavior.
  /// {@endtemplate}
  ...
  @override
  RawScrollbarModifiedState<RawScrollbarModified> createState() => RawScrollbarModifiedState<RawScrollbarModified>();
}

/// The state for a [RawScrollbarModified] widget, also shared by the [Scrollbar] and
...
/// scrollbar track.
class RawScrollbarModifiedState<T extends RawScrollbarModified> extends State<T> with TickerProviderStateMixin<T> {
  ...
  /// theme.
  ///
  /// Defaults to false when [RawScrollbarModified.thumbVisibility] or
  /// [RawScrollbarModified.thumbVisibility] is null.
  ///
  /// See also:
  ///
  ///   * [RawScrollbarModified.thumbVisibility], which overrides the default behavior.
  @protected
  ...
  /// theme.
  ///
  /// Defaults to true when [RawScrollbarModified.interactive] is null.
  ///
  /// See also:
  ///
  ///   * [RawScrollbarModified.interactive], which overrides the default behavior.
  @protected
  ...
  void _updateScrollPosition(Offset updatedOffset) {
    ...
    if (primaryDeltaFromDragStart > 0 && scrollOffsetGlobal < position.pixels || primaryDeltaFromDragStart < 0 && scrollOffsetGlobal > position.pixels) {
    ...
    if (scrollOffsetGlobal != position.pixels) {
      ...
      final step = widget.scrollStep;
      if (step > 0) {
        final rounded = (newPosition / step).floorToDouble();
        newPosition = step * rounded;
      }
  ...
  /// [RawScrollbarModified].
  ///
  /// Excludes the [RawScrollbarModified] thumb.
  @protected
